...
/**
 * Translates {@link RecyclerView.OnScrollListener} events to {@link ViewPager2OSS.OnPageChangeCallback} events
 * for {@link ViewPager2OSS}. As part of this process, it keeps track of the current scroll position
...
final class ScrollEventAdapter extends RecyclerView.OnScrollListener {
    ...
    private ViewPager2OSS.OnPageChangeCallback mCallback;
    private final @NonNull
    ViewPager2OSS mViewPager;
    ...
    private @ViewPager2OSS.ScrollState int mScrollState;
    ...
    ScrollEventAdapter(@NonNull ViewPager2OSS viewPager) {
    ...
    private void resetState() {
        ...
        mScrollState = ViewPager2OSS.SCROLL_STATE_IDLE;
        mScrollValues.reset();
    ...
    public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
        ...
        if ((mAdapterState != STATE_IN_PROGRESS_MANUAL_DRAG
                || mScrollState != ViewPager2OSS.SCROLL_STATE_DRAGGING)
        ...
        if (isInAnyDraggingState() && newState == RecyclerView.SCROLL_STATE_SETTLING) {
            ...
            if (mScrollHappened) {
                dispatchStateChanged(ViewPager2OSS.SCROLL_STATE_SETTLING);
        ...
        if (isInAnyDraggingState() && newState == RecyclerView.SCROLL_STATE_IDLE) {
            ...
            if (dispatchIdle) {
                ...
                dispatchStateChanged(ViewPager2OSS.SCROLL_STATE_IDLE);
                resetState();
            }
        }
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        if ((mScrollValues.mPosition == mTarget || mTarget == NO_POSITION)
                && mScrollValues.mOffsetPx == 0 && !(mScrollState == ViewPager2OSS.SCROLL_STATE_DRAGGING)) {
    ...
    private void startDrag(boolean isFakeDrag) {
        ...
        dispatchStateChanged(ViewPager2OSS.SCROLL_STATE_DRAGGING);
    }
    ...
    void setOnPageChangeCallback(ViewPager2OSS.OnPageChangeCallback callback) {
        mCallback = callback;
    }
     ...
     */
    boolean isIdle() {
        return mScrollState == ViewPager2OSS.SCROLL_STATE_IDLE;
    }
     ...
     */
    boolean isDragging() {
        return mScrollState == ViewPager2OSS.SCROLL_STATE_DRAGGING;
    }

    /**
     * @return {@code true} if a fake drag is ongoing. Returns {@code false} from the moment the
     *         {@link ViewPager2OSS#endFakeDrag()} is called.
     */
     ...
     * the ViewPager is settling towards its {@link ViewPager2OSS#getCurrentItem() current item}, or
    ...
    private void dispatchStateChanged(@ViewPager2OSS.ScrollState int state) {
