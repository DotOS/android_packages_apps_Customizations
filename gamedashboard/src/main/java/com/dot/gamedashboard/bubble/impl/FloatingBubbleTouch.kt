...
class FloatingBubbleTouch private constructor(builder: Builder) : View.OnTouchListener {
    ...
    private fun moveBubbleView(motionEvent: MotionEvent) {
        val width = 1
        val halfClipSize = (width / 2).toFloat()
        val clipSize = width.toFloat()
        ...
        bubbleParams.x = leftX.toInt() / 2
        bubbleParams.y = topY.toInt() / 8
        //handleRemove()
    ...
    init {
        ...
        removeBubbleParams = bubbleView!!.layoutParams as WindowManager.LayoutParams
        expandableParams = bubbleView!!.layoutParams as WindowManager.LayoutParams
