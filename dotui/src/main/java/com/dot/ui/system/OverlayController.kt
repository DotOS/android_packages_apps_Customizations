...
import com.dot.ui.system.themes.*
...
class OverlayController(
    ...
    inner class AccentColors {
        ...
        fun setOverlay(
            packageName: String,
            accent: AccentColor,
            holder: AccentPickerV2Adapter.ViewHolder,
        ): Boolean {
        ...
        private fun updateSelection(accent: AccentColor, holder: AccentPickerV2Adapter.ViewHolder) {
            if (accent.selected) {
        ...
        fun getAccentColorsV2(): ArrayList<AccentColor> {
            val accentPacks = ArrayList<AccentColor>()
            val selectedPkg = OVERLAY_TARGET_PACKAGE
            val accentColor = AccentColor()
            accentColor.colorDark = getDarkAccentColor(selectedPkg)
            accentColor.colorLight = getLightAccentColor(selectedPkg)
            accentColor.packageName = selectedPkg
            accentPacks.add(accentColor)
            for (overlayInfo in getOverlayInfos()) {
                val accentColor = AccentColor()
                accentColor.colorDark = getDarkAccentColor(overlayInfo.packageName)
                accentColor.colorLight = getLightAccentColor(overlayInfo.packageName)
                accentColor.packageName = overlayInfo.packageName
                accentPacks.add(accentColor)
