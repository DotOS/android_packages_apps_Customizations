...
fun PackageManager.overlayController(category: String): OverlayController = OverlayController(
    category, this, IOverlayManager.Stub.asInterface(ServiceManager.getService(Context.OVERLAY_SERVICE))
)
...
fun String.deserialize(): Any? {
    return ObjectSerializer.deserialize(this)
}

object ResourceHelper {

    @SuppressLint("WrongConstant")
    fun getAppLockManager(context: Context): AppLockManager {
        return context.getSystemService(Context.APPLOCK_SERVICE) as AppLockManager
    }

    fun getAspectRatio(context: Context): AspectRatio {
    ...
    class AspectRatio(val a1: Int, val a2: Int) {
        init {
            Log.d("AspectRatio", "a1 = $a1, a2 = $a2")
        }
    }
    ...
    fun saveMediaToStorage(
        context: Context,
        bitmap: Bitmap,
        fileName: String = "dotWalls-${Random.nextInt(100000, 999999)}",
        callback: (() -> Unit)? = null
    ) {
     ...
     */
    fun getToleratedShade(color: Int, shade: TwoToneAccentView.Shade): TwoToneAccentView.Shade {
        val tolerance = 0.3
        val colorDarkness = getDarkness(color)
        var toleratedDarkness = colorDarkness
        if (shade == TwoToneAccentView.Shade.LIGHT)
            toleratedDarkness -= tolerance
        if (shade == TwoToneAccentView.Shade.DARK)
            toleratedDarkness += tolerance
        return if (toleratedDarkness in 0.0..0.5) TwoToneAccentView.Shade.LIGHT else TwoToneAccentView.Shade.DARK
    }
    ...
    fun hasNavbarByDefault(context: Context): Boolean {
        var needsNav =
            context.resources.getBoolean(com.android.internal.R.bool.config_showNavigationBar)
    ...
    fun isNavbarEnabled(context: Context): Boolean {
        ...
        return featureManager.System()
            .getInt("navigation_bar_show", if (hasNavbarByDefault(context)) 1 else 0) != 0
    ...
    fun hasButtonBacklightSupport(context: Context): Boolean {
        val buttonBrightnessControlSupported =
            getInternalInteger("config_deviceSupportsButtonBrightnessControl", context) != 0
        return (buttonBrightnessControlSupported && (hasHomeKey(context) || hasBackKey(context) || hasMenuKey(
            context
    ...
    fun getInternalInteger(res: String, context: Context): Int {
        return try {
            context.resources.getInteger(
                Resources.getSystem().getIdentifier(res, "integer", "android")
            )
        } catch (e: Resources.NotFoundException) {
            -1
        }
    ...
    fun getInternalBool(res: String, context: Context): Boolean {
        return try {
            context.resources.getBoolean(
                Resources.getSystem().getIdentifier(res, "bool", "android")
            )
        } catch (e: Resources.NotFoundException) {
            false
        }
